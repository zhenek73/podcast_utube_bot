...
3. In Railway dashboard → Variables, add new variable:
   ...
   - **Important:** When pasting multi-line content, make sure each line ends with `\n` (the bot will automatically convert `\n` to actual newlines)
4. Save and Railway will automatically redeploy
5. The bot will use cookies from the environment variable

**Note:** The bot automatically handles line breaks by converting literal `\n` to actual newlines, so you can paste multi-line cookies content directly.
